...
def compute_cosine_similarity(result: list, index_name: str):
    logging.info("Computing cosine similarity between text and llm answer vector")
...
def compute_sentiment(result: list, hugginface_model_path: str, index_name: str):
    logging.info(f"Downloading {hugginface_model_path} from Huggingface for classifying sentiment.")
...
def compute_llm_as_a_judge(result: list, openai_model_name: str, index_name: str):
    logging.info("Using LLM as a judge to assess suitability of llm answer for the corresponding question.")
    for entry in tqdm(result):
        prompt = f"""
